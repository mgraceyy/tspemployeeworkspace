...
use crate::services::leave_balances::{
    assert_sufficient_balance, deduct_for_approved_request_in_tx,
...
pub async fn create_request(
    ...
    assert_sufficient_balance(pool, employee_id, leave_type, start_date, end_date).await?;
...
async fn revoke_approved_leave_in_tx(
    ...
    while day <= request.end_date {
        clear_leave_absence_for_employee_in_tx(
            tx,
            request.employee_id,
            day,
            attendance,
