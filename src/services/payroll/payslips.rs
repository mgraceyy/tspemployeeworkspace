...
pub struct PayslipLineRow {
    ...
    pub lwop_days: i32,
    pub employed_days: Option<i32>,
    pub period_calendar_days: Option<i32>,
...
async fn fetch_payslip_line(pool: &PgPool, line_id: Uuid) -> AppResult<PayslipLineRow> {
    sqlx::query_as::<_, PayslipLineRow>(
        "SELECT l.id AS line_id, r.id AS run_id, l.employee_id, e.employee_code, e.full_name,
                ...
                l.regular_minutes, l.approved_ot_minutes, l.no_show_days, l.lwop_days,
                l.employed_days, l.period_calendar_days,
...
fn row_to_detail(
    ...
    Ok(PayslipDetail {
        ...
        employed_days: row.employed_days,
        period_calendar_days: row.period_calendar_days,
        ...
        premium_pay_cents: row.premium_pay_cents,
