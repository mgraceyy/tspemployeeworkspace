...
pub async fn save_settings(
    ...
    let result: AppResult<()> = async {
        ...
        let pay_period_anchor =
            parse_date(&form.pay_period_anchor).map_err(AppError::bad_request)?;
    ...
    redirect_with_flash_from_result(&session, "/admin/settings", "Settings saved", result).await
}
